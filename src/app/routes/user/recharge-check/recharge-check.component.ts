...
@Component({
  selector: 'app-user-recharge-check',
  templateUrl: './recharge-check.component.html',
  styleUrls: ['./recharge-check.component.less']
})
export class UserRechargeCheckComponent implements OnInit {
  public page_index = 1;
...
*/
  get_passport_aply_list(page_index, data?) {
    ...
    this.userManageService.get_recharge_check_list(page_index, data).subscribe((res: any) => {
      if (res && res.success) {
  ...
  submit_pass() {
    ...
    this.userManageService.submit_recharge_result(option,this.edit_check_obj['type'] === 'pass' ? 1 : 2).subscribe((res: any) => {
      this.submit_passport_lodding = false;
      if (res && res.success) {
        this.note_value = '';
